...
public class SpawnData extends Config {
	...
	public SpawnData() {
		super(new File(Core.getInstance().getDataFolder() + "/spawn.yml"));
	}

	public void onCreate() {
		File file = new File(Core.getInstance().getDataFolder() + "/spawn.yml");
		if (!file.exists()) {
			try {
				file.createNewFile();
			} catch (IOException e) {
	...
	public Location getSpawn() {
		...
		World world = LocationUtils.findWorld(super.getString("Spawn.world"));
