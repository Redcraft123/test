...
public class Config {
	...
	public void save() {
		if(this.config==Bukkit.getPluginManager().getPlugin("MinparcCore").getConfig()) {
			Bukkit.getPluginManager().getPlugin("MinparcCore").saveConfig();
		}else {
			try {
	...
	public static void folders() {
		...
		File file2 = new File(Core.getInstance().getDataFolder()+"/data/users");
		if(!file.exists()) {
			file.mkdir();
		}
		if(!file2.exists()) {
	...
	public static void setDatabase() {
		...
		File file = new File(Core.getInstance().getDataFolder()+"/data/"+config.getDBName());
		if (!file.exists()) {
			try {
				file.createNewFile();
			} catch (IOException e) {
