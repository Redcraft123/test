...
public class LocationUtils {
	...
	public LocationUtils(double x, double y, double z, World world) {
	...
	public Location getTop() {
		...
		for (int i = y; i < 256; i++) {
			loc.setY(i);
			loc2.setY(i - 1);
			if (loc.getBlock().getType().getId() == 0 && loc2.getBlock().getType().getId() != 0) {
				yok = i;
			}
		}
		if (yok != 0) {
	...
	public static World findWorld(String string) {
		Iterator<World> worlds = Core.getInstance().getServer().getWorlds().iterator();
